...
class XlsxOpenpyxlParityTests(unittest.TestCase):
    ...
    def test_corpus_report_skips_only_matching_expected_rxls_rejections(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            expected_path = base / "apache-poi" / "xlsx-corrupted.xlsx"
            unexpected_path = base / "apache-poi" / "unexpected.xlsx"
            perfect_path = base / "perfect.xlsx"
            for path, title, value in [
                (expected_path, "Expected", "oracle"),
                (unexpected_path, "Unexpected", "oracle"),
                (perfect_path, "Perfect", "match"),
            ]:
                ...
                json.dumps(
                    {
                        "files": [
                            {
                                "source": "unit",
                                "path": "xlsx-corrupted.xlsx",
                                "local_path": str(expected_path),
                                "status": "downloaded",
                            },
                            {
                                "source": "unit",
                                "path": "unexpected.xlsx",
                                "local_path": str(unexpected_path),
                                "status": "downloaded",
                            },
                            {
                                "source": "unit",
            ...
            corpus_report.write_text(
                "failure: .xlsx xlsx-corrupted.xlsx "
                "kind=invalid_ooxml_relationships decision=excluded_malformed_workbook "
                "evidence=ooxml_package_relationships_invalid container=zip "
                "extension_mismatch=false parse: invalid office-document relationship "
                "expected=reject:invalid_ooxml_relationships:excluded_malformed_workbook:"
                "ooxml_package_relationships_invalid\n"
                "failure: .xlsx unexpected.xlsx "
                "kind=invalid_ooxml_relationships decision=excluded_malformed_workbook "
                "evidence=ooxml_package_relationships_invalid container=zip "
                "extension_mismatch=false parse: invalid office-document relationship "
            ...
            fake_extract = make_python_stub_executable(fake_extract)
            ...
            self.assertIn("files: 3", output.stdout)
            self.assertIn("rxls extracted: 1", output.stdout)
            self.assertIn("comparable: 2", output.stdout)
            self.assertIn("rxls vs openpyxl: mean parity 50.000%", output.stdout)
            self.assertIn(
                "by_skip_decision: excluded_malformed_workbook skipped=1",
                output.stdout,
            )
            self.assertIn(
                "by_skip_evidence: ooxml_package_relationships_invalid skipped=1",
                output.stdout,
            )
            self.assertIn(
                "by_skip_corpus_kind: invalid_ooxml_relationships skipped=1",
                output.stdout,
            )
            self.assertRegex(
                output.stdout,
                r"skip: kind=corpus-report-excluded decision=excluded_malformed_workbook evidence=ooxml_package_relationships_invalid corpus_kind=invalid_ooxml_relationships path=.*xlsx-corrupted\.xlsx reason=parse: invalid office-document relationship",
            )
            self.assertRegex(output.stdout, r"low-parity: 0\.000 .*unexpected\.xlsx")
            self.assertNotRegex(output.stdout, r"low-parity: .*xlsx-corrupted\.xlsx")
    ...
    def test_shared_strings_path_variants_are_documented_oracle_limitations(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            backslash_variant_path = base / "shared-strings-backslash.xlsx"
            _write_shared_strings_path_variant_workbook(
                backslash_variant_path, "xl\\sharedStrings.xml"
            )
                ...
                json.dumps(
                    {
                        "files": [
                            ...
                            {
                                "source": "unit",
                                "path": "shared-strings-backslash.xlsx",
                                "local_path": str(backslash_variant_path),
                                "status": "downloaded",
                            },
                        ]
                    }
            ...
            self.assertIn("files: 3", output.stdout)
            self.assertIn("rxls extracted: 3", output.stdout)
            self.assertIn("openpyxl-unreadable: 2", output.stdout)
            self.assertIn("comparable: 1", output.stdout)
            self.assertIn(
                "by_skip_decision: documented_oracle_limitation skipped=2",
                output.stdout,
            )
            self.assertIn(
                "by_skip_evidence: openpyxl_shared_strings_path_variant skipped=2",
                output.stdout,
            )
            self.assertIn(
...
""",
        ...
        shared_strings_info = ZipInfo(shared_strings_part.replace("\\", "/"))
        shared_strings_info.filename = shared_strings_part
        shared_strings_info.orig_filename = shared_strings_part
        shared_strings_info.compress_type = ZIP_DEFLATED
        package.writestr(
            shared_strings_info,
            """<?xml version="1.0" encoding="UTF-8"?>
