...
class ReleaseToolTests(unittest.TestCase):
    ...
    def test_tag_publication_requires_immutable_exact_sha_candidate_attestation(self) -> None:
        ...
        self.assertIn('attestation.get("comparison_run_attempt")', workflow)
        self.assertIn('attestation.get("baseline_run_attempt")', workflow)
        ...
        candidate_download_index = workflow.index(
            '--name "rxls-${version}-release-${selected_attempt}"', gate_index
        )
    ...
    def test_hygiene_rejects_internal_planning_document_names(self) -> None:
        ...
        paths = [
            ...
            "docs/" + "T" + "RD.md",
            "docs/" + "P" + "RD-private.md",
        ]
        findings = [
            finding
            for path in paths
            for finding in module.scan_internal_planning_name(path)
        ]

        self.assertEqual(
            [finding.kind for finding in findings],
            ["internal_planning_document"] * len(paths),
        )
