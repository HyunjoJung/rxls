...
def _read_result(queue: Any, exitcode: int | None) -> OracleCallResult:
    try:
        return queue.get_nowait()
    except (Empty, EOFError, OSError, ValueError):
        return OracleCallResult("error", error=f"oracle exited with code {exitcode}")
...
def run_with_timeout(
    ...
    return _read_result(queue, process.exitcode)
