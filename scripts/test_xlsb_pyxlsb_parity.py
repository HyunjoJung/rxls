...
class XlsbPyxlsbParityTests(unittest.TestCase):
    ...
    def test_expected_values_use_unique_basename_for_small_ci_corpus(self) -> None:
        expected = {
            "calamine/tests/issues.xlsb": ["expected"],
            "calamine/tests/date.xlsb": ["date-a"],
            "apache-poi/date.xlsb": ["date-b"],
        }

        self.assertEqual(
            xlsb_pyxlsb_parity.expected_values_for("local/xlsb/issues.xlsb", expected),
            ["expected"],
        )
        self.assertIsNone(
            xlsb_pyxlsb_parity.expected_values_for("local/xlsb/date.xlsb", expected)
        )
