...
def parse_corpus_report(path):
    ...
    with open(path, encoding="utf-8") as report:
        for line in report:
            ...
            if not match:
                continue
            failure = match.groupdict()
            error, separator, expected = failure["error"].rpartition(" expected=")
            if separator and (expected == "classification" or expected.startswith("reject:")):
                failure["error"] = error
                failure["expected"] = expected
            else:
                failure["expected"] = None
...
def corpus_failure_is_expected(failure):
    expected = "reject:{kind}:{decision}:{evidence}".format(**failure)
    return failure.get("expected") == expected
...
def skip_classification(kind, reason, path=None, corpus_failures=None):
    if kind == "corpus-report-excluded":
        ...
        if failure and corpus_failure_is_expected(failure):
            return (failure["decision"], failure["evidence"], failure["kind"])
        return ("needs_oracle_triage", "missing_expected_corpus_failure", None)
...
def main():
    ...
    ap.add_argument(
        "--corpus-report",
        default=None,
        help="optional `rxls corpus-report` output used to exclude expected rxls rejections and refine openpyxl skips",
    )
    ...
    for f in files:
        ...
        if not rt.strip() and failure and corpus_failure_is_expected(failure):
