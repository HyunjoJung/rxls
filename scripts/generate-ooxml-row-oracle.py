#!/usr/bin/env python3
"""Generate the deterministic project-owned OOXML implicit-row oracle matrix.

The twelve XLSX workbooks in this diagnostic corpus are intentionally separate
from the pilot/full render corpus. They isolate SpreadsheetML implicit row
height behavior without changing the release lattice or its reviewed hashes.
Generated files remain local-only below ``local/render-corpus-generated``.
"""
...
<styleSheet xmlns="{SHEET_NS}">
  <fonts count="1"><font><sz val="{size}"/><name val="{family}"/><family val="2"/></font></fonts>
...
def _worksheet(spec: CaseSpec) -> str:
    ...
    right_to_left = (
        ' rightToLeft="1"' if spec.toggle == "right-to-left-layout" else ""
    )
    row_four = ""
    ...
    drawing = (
        '<drawing r:id="rIdDrawing"/>' if spec.toggle == "image-drawing" else ""
    )
    return f"""<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
def build_case(spec: CaseSpec) -> bytes:
    ...
    drawing = spec.toggle == "image-drawing"
    defaults = [
    ...
    if drawing:
        parts.extend(
            (
                ...
                ("xl/drawings/drawing1.xml", _drawing_xml()),
                (
...
def _validate_package(spec: CaseSpec, payload: bytes) -> None:
    ...
    name = font.find("s:name", namespace)
    size = font.find("s:sz", namespace)
    if (
        name is None
        or size is None
    ...
    if view is None or (
        ("rightToLeft" in view.attrib)
        != (spec.toggle == "right-to-left-layout")
    ):
        raise OracleCorpusError("RTL feature mismatch")
    drawing = sheet.find("s:drawing", namespace)
    if (
        (drawing is not None) != (spec.toggle == "image-drawing")
        or has_drawing_parts != (spec.toggle == "image-drawing")
    ):
