...
use crate::scene::{
    ...
    TextAnchor, TextBaseline, TextStyle, FIXED_UNITS_PER_PIXEL,
};
...
fn build_page_scene(
    ...
    let contiguous_page_range =
        contiguous_page_render_range(slot, &repeat_row_ranges, &repeat_col_ranges);
    ...
    for &(first_row, last_row) in &repeat_row_ranges {
        ...
        if contiguous_page_range.is_none() && slot.columns.size.raw() > 0 {
            append_block(
                &mut nodes,
                sheet,
                sheet_index,
    ...
    for (first_col, last_col) in repeat_column_order {
        ...
        if contiguous_page_range.is_none() && slot.rows.size.raw() > 0 {
            append_block(
                &mut nodes,
                sheet,
                sheet_index,
    ...
    for &((first_row, last_row), y, height) in &placed_repeat_rows {
        for &((first_col, last_col), x, width) in &placed_repeat_columns {
            if contiguous_page_range.is_some() {
                continue;
            }
            append_block(
                &mut nodes,
...
fn contiguous_page_render_range(
    slot: PageSlot,
    repeat_rows: &[(u32, u32)],
    repeat_columns: &[(u16, u16)],
) -> Option<RenderRange> {
    let rows = contiguous_row_range(
        (slot.rows.size.raw() > 0).then_some((slot.rows.first, slot.rows.last)),
        repeat_rows,
    )?;
    let columns = contiguous_column_range(
        (slot.columns.size.raw() > 0).then_some((slot.columns.first, slot.columns.last)),
        repeat_columns,
    )?;
    Some(RenderRange::new(rows.0, columns.0, rows.1, columns.1))
}

fn contiguous_row_range(body: Option<(u32, u32)>, repeats: &[(u32, u32)]) -> Option<(u32, u32)> {
    let mut ranges = repeats.to_vec();
    ranges.extend(body);
    ranges.sort_unstable();
    let (first, mut last) = *ranges.first()?;
...
fn contiguous_column_range(body: Option<(u16, u16)>, repeats: &[(u16, u16)]) -> Option<(u16, u16)> {
    let mut ranges = repeats.to_vec();
    ranges.extend(body);
    ranges.sort_unstable();
    let (first, mut last) = *ranges.first()?;
        ...
        SceneNode::GlyphRun(run) if !run.text.is_empty() && !run.clusters.is_empty() => {
            ...
            if divider_index == Some(0) && run.semantic_groups.len() == 2 {
                run.semantic_groups.insert(
                    ...
                    GlyphSemanticGroup {
                        source_start: 0,
                        source_end: u64::try_from(run.text.len())
                            .map_err(|_| RenderError::CoordinateOverflow)?,
                    },
                );
            }
            if !run.metadata_is_valid() {
                return Err(RenderError::Backend {
...
mod tests {
    ...
    #[test]
    fn authored_print_prefixes_retention_without_discarding_visual_lines() {
        fn glyph_run<'a>(
            nodes: &'a [SceneNode],
            expected: &str,
        ) -> Option<&'a crate::scene::GlyphRunNode> {
            nodes.iter().find_map(|node| match node {
                SceneNode::ClipGroup(group) => glyph_run(&group.nodes, expected),
                SceneNode::GlyphRun(run) if run.text == expected => Some(run),
                _ => None,
            })
        }

        const TEXT: &str = "authored print logical text crosses its cell";
        for right_to_left in [false, true] {
            let pack = crate::font::synthetic_test_pack();
            let mut workbook = Workbook::new();
            let sheet = workbook.add_sheet("clipped semantics");
            sheet.set_right_to_left(right_to_left);
            sheet.write(0, 0, TEXT);
            sheet.write(0, 1, "boundary");
            sheet.set_col_width(0, 4.0);
            sheet.set_col_width(1, 4.0);
            sheet.set_page_setup(PageSetup::new().with_print_area((0, 0, 0, 1)));
            ...
            let ordinary = build_sheet_scene(&workbook.sheets[0], 0, &render).unwrap();
            let ordinary_run = glyph_run(&ordinary.scene.nodes, TEXT).expect("ordinary glyph run");
            let ordinary_divider = ordinary_run
                .semantic_groups
                .iter()
                .position(|group| group.source_start == group.source_end)
                .expect("ordinary run semantic layout divider");
            let ordinary_layout_records = ordinary_run.semantic_groups[ordinary_divider..].to_vec();
            assert!(
                ordinary_layout_records.len() > 1,
                "ordinary layout must retain at least one visual-line record"
            );
            let ordinary_command_count = ordinary_run.commands.len();
            ...
            .unwrap();
            let print_run =
                glyph_run(&document.pages[0].scene.nodes, TEXT).expect("authored-print glyph run");
            assert_eq!(
                print_run.semantic_groups.first(),
                Some(&GlyphSemanticGroup {
                    source_start: 0,
                    source_end: u64::try_from(TEXT.len()).unwrap(),
                }),
                "RTL={right_to_left}: authored print must retain the complete logical run"
            );
            assert_eq!(
                &print_run.semantic_groups[1..],
                ordinary_layout_records.as_slice(),
                "RTL={right_to_left}: authored print must preserve the divider and visual-line records"
            );
            assert_eq!(
                print_run.commands.len(),
                ordinary_command_count,
                "RTL={right_to_left}: semantic retention must not synthesize paint commands"
            );
            assert!(print_run.metadata_is_valid());
        }
    }

    #[test]
    fn authored_print_preserves_rtl_sheet_mixed_script_clipping_groups() {
        fn glyph_run<'a>(
            nodes: &'a [SceneNode],
            expected: &str,
        ) -> Option<&'a crate::scene::GlyphRunNode> {
        ...
        const TEXT: &str = "مرحبا بالعالم 0007";
        let pack = crate::font::synthetic_test_pack();
        let mut workbook = Workbook::new();
        let sheet = workbook.add_sheet("mixed RTL print");
        sheet.set_right_to_left(true);
        for column in 0..4 {
            sheet.write(0, column, "blocker");
            sheet.set_col_width(column, 4.0);
        }
        sheet.write(0, 4, TEXT);
        sheet.set_col_width(4, 4.0);
        sheet.set_page_setup(PageSetup::new().with_print_area((0, 0, 0, 4)));
        let render = RenderOptions {
            font_pack: Some(pack.clone()),
            default_font_family: pack.default_family().to_string(),
            gridlines: false,
        ...
        let ordinary = build_sheet_scene(&workbook.sheets[0], 0, &render).unwrap();
        let ordinary_run = glyph_run(&ordinary.scene.nodes, TEXT).expect("ordinary glyph run");
        let ordinary_groups = ordinary_run.semantic_retention_groups().to_vec();
        assert_eq!(ordinary_groups.len(), 5);
        ...
        let print_run =
            glyph_run(&document.pages[0].scene.nodes, TEXT).expect("authored-print glyph run");
        assert_eq!(print_run.semantic_retention_groups(), ordinary_groups);
        assert_eq!(
            print_run.semantic_line_records(),
            ordinary_run.semantic_line_records(),
            "authored print must not replace RTL-sheet scalar clipping metadata"
        );
        assert!(print_run.metadata_is_valid());
    }
    ...
    fn paginated_body_replays_prepared_nonlocal_auto_row_geometry() {
        ...
        let expected_row_height = scale_fixed(
            areas[0].measured_rows[0].size,
            prepared.report.pages[0].scale_permille,
        )
        .unwrap();
        ...
        assert_eq!(first_body.1.height, expected_row_height);
        let visible_body = intersect_rect(first_body.0, first_body.1)
            .unwrap()
            .expect("filled body intersects the global content clip");
        assert_eq!(visible_body.y, first_body.1.y);
        assert_eq!(visible_body.height, first_body.1.height);
    }
