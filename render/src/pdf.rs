...
use crate::scene::{
    ...
    GlyphClusterMetrics, GlyphRunNode, GlyphSemanticLayout, ImageNode, LineNode, PathCommand,
    PathNode, Rect, RectNode, Rgb, Scene, SceneNode, TextAnchor, TextBaseline, TextNode,
...
fn glyph_spans_are_on_distinct_lines(
    ...
    if let (Some(left), Some(right)) = (left.visual_line_id, right.visual_line_id) {
        return left != right;
    }
...
fn nominal_rect_as_pdf_bounds(rect: Rect) -> Option<PdfGlyphBounds> {
    Some(PdfGlyphBounds {
        min_x: rect.x,
        min_y: rect.y,
        max_x: rect.x.checked_add(rect.width)?,
        max_y: rect.y.checked_add(rect.height)?,
    })
}
...
fn semantic_box_and_baselines_intersect_clip(
    ...
    if bounds.width <= Fixed::ZERO
        || bounds.height <= Fixed::ZERO
        || clip.width <= Fixed::ZERO
        || clip.height <= Fixed::ZERO
    {
        return false;
    }
    let Some(bounds_for_clip) = nominal_rect_as_pdf_bounds(bounds) else {
        return false;
    };
    if !transformed_bounds_intersect_clip(bounds_for_clip, clip, transform) {
        return false;
    }
    if !require_baseline {
        return true;
    }
    let width = bounds.width.max(Fixed::from_raw(1));
    baselines.iter().any(|baseline| {
        ...
        nominal_rect_as_pdf_bounds(band)
            .is_some_and(|band| transformed_bounds_intersect_clip(band, clip, transform))
    })
}
...
fn expand_layout_retention_visibility(
    ...
    if layout.retention_groups.is_empty() {
        return;
    }
    let mut retained = vec![false; layout.retention_groups.len()];
    let overlapping_groups = |cluster: &crate::scene::GlyphCluster| {
        let cluster_start = usize::try_from(cluster.source_start).unwrap_or(usize::MAX);
        let cluster_end = usize::try_from(cluster.source_end).unwrap_or(usize::MAX);
        let start = layout
            .retention_groups
            .partition_point(|group| group.source.end <= cluster_start);
        let end = layout.retention_groups[start..]
            .partition_point(|group| group.source.start < cluster_end)
            + start;
        start..end
    };
    debug_assert_eq!(visible.len(), triggers.len());
    for (cluster, trigger) in node.clusters.iter().zip(triggers) {
        if *trigger {
            retained[overlapping_groups(cluster)].fill(true);
        }
    }
    for (cluster, visible) in node.clusters.iter().zip(visible.iter_mut()) {
        let groups = overlapping_groups(cluster);
        if layout.retention_groups[groups.clone()]
            .iter()
            .zip(&retained[groups])
            .any(|(group, retained)| {
                *retained
                    && group.policy
                        == crate::scene::GlyphSemanticRetentionPolicy::RetainCompleteSourceGroup
            })
        {
            *visible = true;
        }
...
fn authoritative_glyph_semantic_spans(
    ...
    for (line_order, line) in layout.lines.iter().enumerate() {
        ...
        for (word_order, word) in line.words.iter().enumerate() {
            debug_assert_eq!(word.reading_order as usize, word_order);
            debug_assert_eq!(word.visual_line_id, line.visual_line_id);
            debug_assert!(line.source.start <= word.source.start);
            debug_assert!(word.source.end <= line.source.end);
            debug_assert_eq!(
                word.retention,
                if layout.retention_groups.is_empty() {
                    crate::scene::GlyphSemanticRetentionPolicy::ClipToNominalClusters
                } else {
                    crate::scene::GlyphSemanticRetentionPolicy::RetainCompleteSourceGroup
                }
            );
    ...
    for line in &layout.lines {
        for word in &line.words {
            let mut source_ranges = word
                .cluster_indices
                .iter()
                .copied()
                .filter(|index| visible[*index])
                .map(|index| glyph_cluster_source_range(node, index))
                .collect::<Result<Vec<_>, _>>()?;
            source_ranges.sort_unstable_by_key(|source| (source.start, source.end));
            ...
            for source in merged {
                let glyphs = word
                    ...
                    .filter(|index| {
                        ...
                        let cluster = node.clusters[*index];
                ...
                if !glyphs.is_empty() {
                    spans.push(PdfGlyphSemanticSpan {
                        source,
                        glyphs,
                        visual_line_id: Some(line.visual_line_id),
                    });
                }
            }
        }
...
mod tests {
    ...
    fn font_pack_glyph_run(text: &str) -> GlyphRunNode {
        ...
        sheet.write_styled(
            ...
            0,
            text,
            &rxls::CellStyle::new().align(rxls::HAlign::Left),
        );
    ...
    #[test]
    fn type3_semantic_separator_ignores_group_and_paint_only_boundaries() {
        ...
        assert_eq!(
            source.matches("<0020>").count(),
            4,
            "all adjacent semantic cells share flow despite intervening paint and clip nodes"
        );
        if let Some(text) = poppler_text(&pdf) {
    ...
    #[test]
    fn zero_extent_nominal_semantic_boxes_are_not_visible() {
        let clip = Rect {
            x: Fixed::ZERO,
            y: Fixed::ZERO,
            width: Fixed::from_pixels(10),
        ...
        let transform = PdfTransform::rotation(0, Fixed::ZERO, Fixed::ZERO);
        for bounds in [
            Rect {
                x: Fixed::from_pixels(5),
                y: Fixed::from_pixels(2),
                width: Fixed::ZERO,
                height: Fixed::from_pixels(5),
            },
            Rect {
                x: Fixed::from_pixels(2),
                y: Fixed::from_pixels(5),
                width: Fixed::from_pixels(5),
                height: Fixed::ZERO,
            },
        ] {
            assert!(!semantic_box_and_baselines_intersect_clip(
                bounds,
                &[Fixed::from_pixels(5)],
                clip,
                transform,
    ...
    fn type3_poppler_soft_wrapped_cjk_preserves_order_and_line_boxes() {
        ...
        assert!(spans
            .iter()
            .enumerate()
            .all(|(index, span)| span.visual_line_id == u32::try_from(index).ok()));
        let span_text = spans
            .iter()
            .map(|span| &node.text[span.source.clone()])
            .collect::<Vec<_>>();
