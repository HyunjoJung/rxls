...
#[test]
fn streamed_pages_aggregate_decoded_media_across_disjoint_repeat_blocks() {
    let mut workbook = imported_page_images(&[PageImage {
        x_inches: 0.125,
        y_inches: 5.5,
        width_inches: 0.25,
        // Span the repeated first row and the disjoint third-row body page.
        height_inches: 8.5,
    }]);
    workbook.sheets[0].set_page_setup(
        PageSetup::new()
    ...
    assert_eq!(
        image_clips(
            &build_print_page(&workbook, &exact_prepared, 1)
                .unwrap()
                .scene
                .nodes
        )
    ...
    let prepared = prepare_print_document(&workbook, 0, &options).unwrap();
    assert_eq!(
        build_print_page(&workbook, &prepared, 1),
        Err(RenderError::LimitExceeded {
            kind: LimitKind::DecodedMediaBytes,
            limit: 7,
            actual: 8,
