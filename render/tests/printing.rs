...
fn nested_text_clip(nodes: &[SceneNode], expected: &str) -> Option<Rect> {
    nodes.iter().find_map(|node| match node {
        SceneNode::ClipGroup(group) => nested_text_clip(&group.nodes, expected),
        SceneNode::Text(node) if node.text == expected => Some(node.clip_bounds),
        SceneNode::GlyphRun(node) if node.text == expected => Some(node.clip_bounds),
        _ => None,
    })
}
...
#[test]
fn contiguous_print_title_and_body_share_layout_overflow_and_clip() {
    let mut workbook = Workbook::new();
    let sheet = workbook.add_sheet("Shared print layout");
    sheet.write(0, 0, "title text crosses empty body cells");
    sheet.write(0, 2, "body boundary");
    for column in 0..=2 {
        sheet.set_col_width(column, 4.0);
    }
    sheet.set_page_setup(
        PageSetup::new()
            .with_print_area((0, 0, 0, 2))
            .with_repeat_cols(0, 0),
    );
    ...
    let content_groups = page
        .scene
        .nodes
        .iter()
        .filter(|node| matches!(node, SceneNode::ClipGroup(_)))
        .count();
    assert_eq!(
        content_groups, 1,
        "one contiguous page needs one layout clip"
    );

    let clip = nested_text_clip(&page.scene.nodes, "title text crosses empty body cells")
        .expect("title text node");
    assert!(
        clip.width > Fixed::from_pixels(32),
        "the title cell must retain overflow into the contiguous body: {clip:?}"
    );
}
