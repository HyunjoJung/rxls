...
> **Development status:** Once `v0.1.3` is tagged, later commits may contain
> additive, unreleased work. The installation commands below select the
> versioned package; build the checked-out source to evaluate its exact APIs
> and behavior.
...
After publication, add the exact `0.1.3` library:
