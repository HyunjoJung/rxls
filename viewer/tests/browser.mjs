...
try {
  ...
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()).includes(
        "Q3 Operations Snapshot",
      ),
    "initial cell inspector",
  );
  await page.getByRole("tab", { name: "View", exact: true }).click();
  assert.equal(await page.locator("#ribbon-home").isHidden(), true);
  await page.locator("#toggle-inspector").click();
  assert.equal(await page.locator("#cell-inspector").isHidden(), true);
  assert.ok(
    (await page.locator("#viewer-viewport").boundingBox()).height > 200,
  );
  await page.locator("#toggle-inspector").click();
  await page.locator("#reset-zoom").click();
  assert.equal(await page.locator("#zoom-value").textContent(), "100%");
  await page.getByRole("tab", { name: "View", exact: true }).press("ArrowLeft");
  assert.equal(
    await page
  ...
  await page.locator("#inspector-reference").fill("XFE1");
  ...
  await waitForCondition(
    ...
    "invalid inspector reference",
  );
  assert.equal(await page.locator("#inspector-edit").isDisabled(), true);
  await page.locator("#inspector-reference").fill("a1");
  await page.locator("#inspector-reference").press("Enter");
  await waitForCondition(
    async () => !(await page.locator("#inspector-edit").isDisabled()),
    "loaded inspector cell",
  );
  assert.equal(await page.locator("#inspector-reference").inputValue(), "A1");
  await page.locator("#edit-cell").click();
  await page.locator("#cell-dialog").waitFor({ state: "visible" });
  assert.equal(await page.locator("#cell-reference").inputValue(), "A1");
  ...
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()) === "420000",
    "numeric cell inspection",
  );
  await page.locator("#edit-cell").click();
  await waitForCondition(
  ...
  await page.locator("#cell-value").fill("-12.5");
  await page.locator("#apply-cell-edit").click();
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()) === "-12.5",
    "numeric edit reflected in inspector",
  );
  assert.match(
    await page.locator("#workbook-mode").textContent(),
    /Unsaved changes/,
  );
  ...
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()) === "420000",
    "numeric undo reflected in inspector",
  );
  ...
  await page.locator("#cell-formula").fill("=C5*2");
  await page.locator("#cell-cached-kind").selectOption("number");
  await page.locator("#cell-cached-value").fill("370000");
  await page.locator("#apply-cell-edit").click();
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()) === "=C5*2",
    "formula expression reflected in inspector",
  );
  await page.locator("#undo-edit").click();
  await waitForCondition(
    async () =>
      (await page.locator("#inspector-value").inputValue()) === "420000",
    "formula undo reflected in inspector",
  );
  ...
  if (
    ...
    throw new Error(
      `unexpected initial viewer state: ${JSON.stringify(state)}`,
    );
  }
  ...
  await selectSample(page, "legacy-korean");
  await delayedLegacyStarted.promise;
  await selectSample(page, "binary-workbook");
  releaseDelayedLegacy.resolve();
  await waitForViewerState(
    page,
    (snapshot) =>
      snapshot.format === "xlsb" && snapshot.rendered && !snapshot.busy,
    "latest sample request",
  );
  await delayedLegacyFinished.promise;
  assert.equal(
    (await page.evaluate(() => globalThis.__rxlsViewerState())).format,
    "xlsb",
  );
  ...
  await selectSample(page, "open-document");
  await waitForViewerState(
    page,
    (snapshot) =>
      snapshot.format === "ods" && snapshot.rendered && !snapshot.busy,
    "latest worker open",
  );
  await waitForCondition(
  ...
  await waitForViewerState(
    page,
    (snapshot) =>
      snapshot.format === "xlsx" && snapshot.rendered && !snapshot.busy,
  ...
  await waitForCondition(
    async () =>
      (await page.locator("#cell-current-value").textContent()).startsWith(
        "B1:",
  ...
  if (process.env.RXLS_VIEWER_SCREENSHOTS) {
    ...
    await page.screenshot({
      path: fileURLToPath(
        new URL("../../target/viewer-e2e/edit-dialog.png", import.meta.url),
      ),
      fullPage: true,
    });
  }
  ...
  assert.match(
    await page.locator("#document-surface").textContent(),
    /Browser edited XLSX/,
  );
  ...
  await Promise.all([
    ...
    selectSample(page, "legacy-korean"),
  ]);
  assert.equal(
    await page.locator("#sample-select").inputValue(),
    "operations-report",
  );
  await waitForViewerState(
    page,
    (snapshot) =>
      snapshot.format === "xlsx" && snapshot.dirty && !snapshot.busy,
  ...
  for (const sample of [
    ...
    assert.equal(
      await page.locator("#editing-reason").textContent(),
      sample.message,
    );
    assert.equal(
    ...
    if (process.env.RXLS_VIEWER_SCREENSHOTS && sample.id === "legacy-korean") {
      await page.screenshot({
        path: fileURLToPath(
          new URL("../../target/viewer-e2e/read-only.png", import.meta.url),
        ),
        fullPage: true,
      });
    }
  ...
  await selectSample(page, "macro-preservation");
  await waitForViewerState(
    page,
    (snapshot) =>
  ...
  await waitForViewerState(
    page,
    (snapshot) =>
      snapshot.source === "Local file" && snapshot.rendered && !snapshot.busy,
    "local workbook",
  );
  ...
  if (process.env.RXLS_VIEWER_SCREENSHOTS) {
    ...
    await page.screenshot({
      path: fileURLToPath(
        new URL("../../target/viewer-e2e/desktop.png", import.meta.url),
      ),
      fullPage: true,
    });
  }
  ...
  await page
    .locator("body.sidebar-open #sidebar")
    .waitFor({ state: "visible" });
  await page.waitForTimeout(250);
  const sidebarBox = await page.locator("#sidebar").boundingBox();
  if (!sidebarBox || sidebarBox.x < -1 || sidebarBox.width > 390) {
    throw new Error(
      `mobile sidebar is outside the viewport: ${JSON.stringify(sidebarBox)}`,
    );
  }
  ...
  if (process.env.RXLS_VIEWER_SCREENSHOTS) {
    await page.screenshot({
      path: fileURLToPath(
        new URL("../../target/viewer-e2e/mobile.png", import.meta.url),
      ),
      fullPage: true,
    });
  }
  ...
  if (
    ...
    throw new Error(
      `mobile edit dialog is outside the viewport: ${JSON.stringify(dialogBox)}`,
    );
  }
  if (process.env.RXLS_VIEWER_SCREENSHOTS) {
    await page.screenshot({
      path: fileURLToPath(
        new URL(
          "../../target/viewer-e2e/mobile-edit-dialog.png",
          import.meta.url,
        ),
      ),
      fullPage: true,
  ...
  if (consoleErrors.length > 0) {
    throw new Error(
      `viewer logged console errors: ${JSON.stringify(consoleErrors)}`,
    );
  }
...
async function exerciseInlineEditing(page) {
  assert.equal(
    await page.locator("#grid-input").count(),
    1,
    "direct cell input exists",
  );
  const input = page.locator("#grid-input");
  const selection = page.locator("#grid-selection");
  const select = async (reference, expected) => {
    await page.locator("#inspector-reference").fill(reference);
    await page.locator("#inspector-reference").press("Enter");
    await waitForCondition(
      async () =>
        (await selection.getAttribute("data-reference")) === reference,
      `selected ${reference}`,
    );
  ...
  const undo = async () => {
    ...
    await waitForViewerState(
      ...
      "inline undo restores source",
    );
  };

  // Click an actual painted value; hit-testing must resolve source C4, not a guessed grid.
  const valueBox = await page
    .locator("svg text")
    .filter({ hasText: /^\$420,000$/ })
    .first()
    .boundingBox();
  assert.ok(valueBox, "numeric value has visible geometry");
  await page.mouse.click(
    valueBox.x + valueBox.width / 2,
    valueBox.y + valueBox.height / 2,
  );
  await waitForCondition(
    async () => (await selection.getAttribute("data-reference")) === "C4",
    "pointer hit C4",
  );
  await waitForCondition(
    async () => !(await input.isDisabled()),
  ...
  assert.equal(
    ...
    "420000",
    "one click opens the existing value",
  );
  assert.match(await input.getAttribute("aria-label"), /C4: Edit cell/);
  assert.equal(
    (await page.evaluate(() => globalThis.__rxlsViewerState())).dirty,
    false,
    "a click alone does not mutate the workbook",
  );
  await input.press(process.platform === "darwin" ? "Meta+a" : "Control+a");
  await page.keyboard.type("125000");
  assert.equal(
    await page.locator("#inspector-value").inputValue(),
    "125000",
    "formula bar follows in-cell typing before commit",
  );
  await input.press("Enter");
  await waitForViewerState(
    page,
    (value) => !value.busy && value.dirty,
    "inline numeric commit",
  );
  await select("C4", "125000");
  assert.ok(
    await page
      .locator("svg text")
      .filter({ hasText: /^\$1,079,000$/ })
      .count(),
    "changing C4 recalculates the dependent SUM total",
  );
  const recalculatedDownload = await downloadWorkbook(page, ".xlsx");
  await assertOpenpyxlReopens(recalculatedDownload.path, {
    expected: "Q3 Operations Snapshot",
    cacheCell: "C10",
    expectedCache: 1079000,
  });
  await undo();
  assert.ok(
    await page
      .locator("svg text")
      .filter({ hasText: /^\$1,374,000$/ })
      .count(),
    "undo restores both the edited value and formula cache",
  );
  ...
  await waitForCondition(
    async () => (await selection.getAttribute("data-reference")) === "D4",
    "right cell navigation",
  );
  await input.press("ArrowDown");
  await waitForCondition(
    async () => (await selection.getAttribute("data-reference")) === "D5",
    "down cell navigation",
  );
  await input.press("Tab");
  await waitForCondition(
    async () => (await selection.getAttribute("data-reference")) === "E5",
    "Tab navigation",
  );
  ...
  assert.equal(await input.inputValue(), "한글 편집");
  await input.dispatchEvent("compositionend", { data: "한글 편집" });
  // Legacy IME key events may report only keyCode 229, including at the global handler.
  await input.dispatchEvent("keydown", {
    key: "Escape",
    keyCode: 229,
    bubbles: true,
  ...
  assert.equal(await input.inputValue(), "한글 편집");
  assert.match(
    await page.locator("#grid-layer").getAttribute("class"),
    /(?:^|\s)is-editing(?:\s|$)/,
  );
  await input.press("Enter");
  await waitForViewerState(
  ...
  await input.fill("=C5*2");
  await input.press("Enter");
  await waitForViewerState(
    page,
    (value) => !value.busy && value.dirty,
    "computed inline formula",
  );
  await select("C4", "=C5*2");
  assert.ok(
    await page
      .locator("svg text")
      .filter({ hasText: /^\$370,000$/ })
      .count(),
    "formula cache is computed",
  );
  await undo();
  ...
  await page.locator("#page-view").click();
  assert.equal(
    (await page.evaluate(() => globalThis.__rxlsViewerState())).mode,
    "sheet",
    "invalid draft blocks context change",
  );
  await waitForCondition(
    async () => !(await input.isDisabled()),
    "rejected context-switch commit settled",
  );
  await page.keyboard.press("Escape");
  await page.locator("#dismiss-error").click();

  // A toolbar save must include an unfinished inline draft, not silently export the prior value.
  ...
  await waitForViewerState(
    page,
    (value) => !value.busy && value.mode === "page",
    "noninteractive print preview",
  );
  assert.equal(await selection.isHidden(), true);
...
async function waitForViewerState(page, predicate, label) {
  ...
  throw new Error(
    `viewer state did not settle for ${label}: ${JSON.stringify(snapshot)}`,
  );
}
...
async function assertOpenpyxlReopens(
  ...
  const { stdout } = await execFileAsync(python, args, { timeout: 30_000 });
  ...
  if (cacheCell) assert.equal(report.cache_value, expectedCache);
  if (expectedTitle !== null) {
...
async function selectSample(page, id) {
  if (
