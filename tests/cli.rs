...
fn rxls_bin() -> String {
    std::env::var("CARGO_BIN_EXE_rxls")
        ...
        .expect("Cargo did not provide the rxls binary path")
}
...
#[cfg(feature = "xlsx")]
#[test]
fn cli_corpus_report_tags_malformed_ooxml_relationships_evidence() {
    use std::io::Write as _;
    use zip::write::SimpleFileOptions;

    let base = std::env::temp_dir().join(format!(
        "rxls_cli_corpus_report_malformed_ooxml_relationships_{}_{}",
        std::process::id(),
        std::thread::current().name().unwrap_or("test")
    ));
    std::fs::create_dir_all(&base).expect("create temp corpus dir");
    let workbook_path = base.join("xlsx-corrupted.xlsx");
    let manifest_path = base.join("manifest.json");

    let mut zip = zip::ZipWriter::new(std::io::Cursor::new(Vec::new()));
    let options = SimpleFileOptions::default();
    zip.start_file("[Content_Types].xml", options)
        .expect("start content types part");
    zip.write_all(
        br#"<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"/>"#,
    )
    .expect("write content types part");
    zip.start_file("_rels/.rels", options)
        .expect("start package relationships part");
    zip.write_all(b"<?xml version=\"1.0\"?>\nCORRUPTED")
        .expect("write malformed package relationships part");
    std::fs::write(&workbook_path, zip.finish().unwrap().into_inner())
        .expect("write malformed OOXML fixture");
    std::fs::write(
        &manifest_path,
        format!(
            r#"{{
  "files": [
    {{"source": "apache-poi", "path": "test-data/spreadsheet/xlsx-corrupted.xlsx", "local_path": "{}", "expected": {{"outcome": "reject", "kind": "invalid_ooxml_relationships", "decision": "excluded_malformed_workbook", "evidence": "ooxml_package_relationships_invalid"}}}}
  ]
}}"#,
    ...
    let output = Command::new(rxls_bin())
        .args([
            "corpus-report",
            manifest_path.to_str().expect("utf8 manifest path"),
            "--limit",
    ...
    assert!(stdout.contains("unexpected_failures: 0"));
    assert!(stdout.contains("by_failure_kind: invalid_ooxml_relationships failed=1"));
    assert!(stdout.contains("by_failure_decision: excluded_malformed_workbook failed=1"));
    assert!(stdout.contains("by_failure_evidence: ooxml_package_relationships_invalid failed=1"));
    assert!(stdout.contains("failure: .xlsx test-data/spreadsheet/xlsx-corrupted.xlsx kind=invalid_ooxml_relationships decision=excluded_malformed_workbook evidence=ooxml_package_relationships_invalid container=zip extension_mismatch=false parse: invalid ZIP package: invalid office-document relationship"));
}
